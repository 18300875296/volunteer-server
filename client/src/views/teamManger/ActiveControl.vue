  ...
  <div class="activesControl">
    ...
    <section class="table">
      <el-table :data="activesData" stripe :border="true">
        <el-table-column prop="active_name" label="活动名称" width="120px">
        ...
        <el-table-column prop="active_start_at" label="开始时间" width="90px">
          <template #default="scope">
            <div class="row_item-text">{{ $formatTime(scope.row.active_start_at) }}</div>
          </template>
        </el-table-column>
        <el-table-column prop="active_end_at" label="结束时间" width="90px">
          <template #default="scope">
            <div class="row_item-text">{{ $formatTime(scope.row.active_end_at) }}</div>
          </template>
        </el-table-column>
        ...
        <el-table-column label="操作">
          <template #default="scope">
            <div class="button-group">
              <cookie-button
                type="collect"
                @click="router.push(`/team_manager/${team_id}/team_active/${scope.row.active_id}/application`)"
                >查看申请</cookie-button
              >
              <cookie-button type="submit" @click="router.push(`/active/${scope.row.active_id}/edit`)"
                >编辑</cookie-button
              >
...
import { Active, Article } from '../../assets/interface';
import { getTeamActivesAPI } from '../../api/team_control';
...
const getActives = async () => {
  const res = await getTeamActivesAPI(props.team_id);
