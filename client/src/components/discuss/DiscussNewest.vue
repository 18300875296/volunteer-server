<template>
  <template v-for="article in newestList" :key="article.article_id">
...
import { getNewestAPI } from '../../api/discuss';

const newestList = ref<Article[]>([]);
const page = ref(0);
...
const useArticle = () => {
  ...
  const getNewArticle = async () => {
    try {
      ...
      newestList.value = data.articles;
  ...
  const getMoreArticle = async () => {
    ...
    if (total.value >= page.value) {
      const { data } = await getNewestAPI(page.value, pageSize.value);
      data.articles.forEach((article: any) => newestList.value.push(article));
    }
  };
  return {
    getNewArticle,
...
const { getNewArticle, getMoreArticle } = useArticle();
